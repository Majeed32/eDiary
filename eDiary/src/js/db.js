...
const initDB = function () {
  const /** {JSON | undefined} */ db = localStorage.getItem('eDiaryDB');

  if (db) {
    eDiaryDB = JSON.parse(db);
  } else {
    eDiaryDB.notebooks = [];
    localStorage.setItem('eDiaryDB', JSON.stringify(eDiaryDB));
  }
}
...
/**
 * Reads and loads the localStorage data in to the global variable `eDiaryDB`.
 */
const readDB = function () {
  eDiaryDB = JSON.parse(localStorage.getItem('eDiaryDB'));
}


/**
 * Writes the current state of the global variable `eDiaryDB` to local storage
 */
...
export const db = {
  ...
  post: {
    ...
    notebook(name) {
      ...
      eDiaryDB.notebooks.push(notebookData);
    ...
    note(notebookId, object) {
      ...
      const /** {Object} */ notebook = findNotebook(eDiaryDB, notebookId);
  ...
  update: {
    ...
    note(noteId, object) {
      ...
      const /** {Object} */ oldNote = findNote(eDiaryDB, noteId);
  ...
  delete: {
    ...
    notebook(notebookId) {
      ...
      const /** {Number} */ notebookIndex = findNotebookIndex(eDiaryDB, notebookId);
      eDiaryDB.notebooks.splice(notebookIndex, 1);
